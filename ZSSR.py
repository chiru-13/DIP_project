...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        self.gt = ground_truth if isinstance(ground_truth, np.ndarray) else img.imread(ground_truth) if ground_truth else None

        # Preprocess kernels
        self.kernels = preprocess_kernels(kernels, conf)

        # Initialize the model with the appropriate input channels
        self.model = simpleNet(self.Y)

        # Initialize parameters and settings
        self.init_parameters()
        self.hr_fathers_sources = [self.input]
        self.file_name = input_img if isinstance(input_img, str) else conf.name
    ...
    def forward_backward_pass(self, lr_son, hr_father, criterion, optimizer):
        # Interpolate input to match size of hr_father
        interpolated_lr_son = imresize(lr_son, self.sf, hr_father.shape, self.conf.upscale_method)

        # Handle grayscale (1-channel) vs. RGB (3-channel) input
        if self.Y:
            lr_son_input = torch.Tensor(interpolated_lr_son).unsqueeze(0).unsqueeze(0)  # [1, 1, H, W]
            hr_father = torch.Tensor(hr_father).unsqueeze(0).unsqueeze(0)  # [1, 1, H, W]
        else:
            lr_son_input = torch.Tensor(interpolated_lr_son).permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
            hr_father = torch.Tensor(hr_father).permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
    ...
    def plot(self):
        ...
        # Process and show current input and output images
        # Handle lr_son
        if self.lr_son.ndim == 3 and self.lr_son.shape[0] in [1, 3]:
            lr_son_img = np.transpose(self.lr_son, (1, 2, 0))
        else:
            lr_son_img = self.lr_son
        self.lr_son_image_space.imshow(lr_son_img, vmin=0.0, vmax=1.0)

        # Handle train_output (out_image_space)
        if self.train_output.ndim == 3 and self.train_output.shape[0] in [1, 3]:
            train_output_img = np.transpose(self.train_output, (1, 2, 0))
        else:
            train_output_img = self.train_output
        self.out_image_space.imshow(train_output_img, vmin=0.0, vmax=1.0)

        # Handle hr_father
        if self.hr_father.ndim == 3 and self.hr_father.shape[0] in [1, 3]:
            hr_father_img = np.transpose(self.hr_father, (1, 2, 0))
        else:
            hr_father_img = self.hr_father
        self.hr_father_image_space.imshow(hr_father_img, vmin=0.0, vmax=1.0)

        # These lines are needed in order to see the graphics at real time
